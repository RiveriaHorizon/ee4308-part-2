           ...
           int (*H_cost)[maze_utility::BOUNDARY_ARRAY_LIMIT][maze_utility::BOUNDARY_ARRAY_LIMIT])
{
    ...
    while ((new_end_x != start_x) || (new_end_y != start_y))
    {
        ...
        for (int i = -1; i <= 1; i++)
        {
            for (int j = -1; j <= 1; j++)
            {
                if ((*maze)[new_end_y + i][new_end_x + j] != 99 &&
                    (i != 0 || j != 0) &&
                    (new_end_x + j) >= 0 &&
                    (new_end_y + i) >= 0 &&
                    (new_end_x + j) < maze_utility::BOUNDARY_ARRAY_LIMIT &&
                    (new_end_y + i) < maze_utility::BOUNDARY_ARRAY_LIMIT) //check for obstacle & boundary
                {
                    ...
                    else if ((*maze)[new_end_y + i][new_end_x + j] == lowest_F &&
                             (*G_cost)[new_end_y + i][new_end_x + j] < lowest_G) //check for lowest G cost if F cost the same
                    {
                        lowest_F = (*maze)[new_end_y + i][new_end_x + j];
                        lowest_G = (*G_cost)[new_end_y + i][new_end_x + j];
                        new_x = new_end_x + j;
    ...
    while (count != max_count) //increment count to maximum count;
    {
        for (int i = -1; i <= 1; i++)
        {
            for (int j = -1; j <= 1; j++)
            {
                if ((*traceback)[start_y + i][start_x + j] != 99 &&
                    (*traceback)[start_y + i][start_x + j] != 11111 &&
                    (start_x + j) >= 0 &&
                    (start_y + i) >= 0 &&
                    (start_x + j) < maze_utility::BOUNDARY_ARRAY_LIMIT &&
                    (start_y + i) < maze_utility::BOUNDARY_ARRAY_LIMIT) //check for boundary & obstacle
                {
